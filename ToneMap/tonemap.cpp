/*
   ...
   For my final project, I have developed a program to tone map HDR images 
   using the point and region processing filtering. The code implements three 
   different HDR tone mapping operators. The first tone mapping operator 
   will be to gamma correct the image, in its log space of its luminance. 
   The second tone mapping operator separately processes the low-pass and high-pass 
   data of the log channel--which will be built using convolution. And finally, 
   I have modified my convolution-based tone mapper to use a bilateral filter. 
*/
...
// tone mapping with convolution using the bilateral filter
